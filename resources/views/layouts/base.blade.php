                        ...
                        <ul>
                            ...
                            @if(\Illuminate\Support\Facades\Auth::check())
                                @auth
                                    @if(\Illuminate\Support\Facades\Auth::user()->role=='ADM')
                                        <li class="menu-item menu-item-has-children parent">
                                            ...
                                            <ul class="submenu curency">
                                                ...
                                                <li class="menu-item">
                                                    <a title="categories table"
                                                       href="{{route('admin.categories')}}">Categories</a>
                                                </li>

                                                <li class="menu-item">
                                                    <a title="porducts table"
                                                ...
                                                <li class="menu-item">
                                                    <a title="porducts table"
                                                       href="{{route('admin.slider')}}">Maneage slider</a>
                                                </li>
                                    ...
                                    @else
                                        <li class="menu-item menu-item-has-children parent">
                                            <a title="user dashboard" href="#">My Account
                                                ({{\Illuminate\Support\Facades\Auth::user()->name}})<i
                                                    class="fa fa-angle-down" aria-hidden="true"></i></a>
                                            <ul class="submenu curency">
                                                ...
                                                <li>
                                                    <form method="POST" action="{{ route('logout') }}">
                                                        @csrf
                                                        <a href="{{ route('logout') }}" onclick="event.preventDefault(); this.closest('form').submit();">{{ __('Log Out') }}
                                                        </a>
                                                    </form>
                                                </li>
...
<script>
    ...
    window.livewire.on('updateProduct', () => {
        $('#updateProModal').modal('hide');
    })

    window.livewire.on('addedSlider', () => {
        $('#createSliderModal').modal('hide');
    })

    window.livewire.on('updateSlider', () => {
        $('#updateSliderModal').modal('hide');
    })

  window.livewire.on('closeReview', () => {
        $('#reviewModal').modal('hide');
    })
    window.livewire.on('detail', () => {
        location.reload();
    })
...
<script src="https://cdn.tiny.cloud/1/akst6f8q208v3hwu0zetlhld8f8tyaiqexf7b3v5qnj2tce9/tinymce/5/tinymce.min.js"
